// const { saveMsgToHistory, getById } = require('../api/toy/station.service')
...
function setupSocketAPI(http) {
  ...
  gIo.on('connection', socket => {
    ...
    socket.on('chat topic', async topic => {
      ...
      // const toy = await getById(topic)
      // gIo.to(socket.myTopic).emit('get-chat-history', toy.chatHistory)
    })
    socket.on('chat newMsg', async msg => {
      ...
      // await saveMsgToHistory(socket.myTopic, msg)
      // gIo.to(socket.myTopic).emit('chat addMsg', msg)
    })
    socket.on('user-typing', ({ userFullname, userId }) => {
      logger.info(`${{ userFullname }} is typing... [id: ${socket.id}]`)

      gIo.to(socket.myTopic).emit('user-typing', { userFullname, userId })
    })
